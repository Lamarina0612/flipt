...
type EvaluationStore interface {
	ListFlags(ctx context.Context, req *storage.ListRequest[storage.NamespacePredicate]) (storage.ResultSet[*flipt.Flag], error)
	...
	for remaining {
		res, err := srv.store.ListFlags(
			...
			storage.NewListRequest(
				...
				storage.ListWithQueryParamOptions[storage.NamespacePredicate](storage.WithPageToken(nextPage)),
			),
		)
		if err != nil {
