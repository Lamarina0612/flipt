...
func (fis *FSIndexSuite) TestListFlags() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			flags, err := fis.store.ListFlags(context.TODO(), storage.NewListRequest(
				storage.NewNamespace(tc.namespace),
				storage.ListWithQueryParamOptions[storage.NamespacePredicate](storage.WithLimit(5), storage.WithPageToken(tc.pageToken)),
			))
			if tc.listError != nil {
...
func (fis *FSIndexSuite) TestCountNamespaces() {
	...
	namespacesCount, err := fis.store.CountNamespaces(context.TODO(), storage.ReferencePredicate{})
	require.NoError(t, err)
...
func (fis *FSIndexSuite) TestListNamespaces() {
	...
	namespaces, err := fis.store.ListNamespaces(context.TODO(), storage.NewListRequest(storage.ReferencePredicate{},
		storage.ListWithQueryParamOptions[storage.ReferencePredicate](storage.WithLimit(2), storage.WithPageToken("0"))))
	require.NoError(t, err)
...
func (fis *FSIndexSuite) TestListSegments() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			flags, err := fis.store.ListSegments(context.TODO(), storage.NewListRequest(
				storage.NewNamespace(tc.namespace),
				storage.ListWithQueryParamOptions[storage.NamespacePredicate](storage.WithLimit(5), storage.WithPageToken(tc.pageToken)),
			))
			if tc.listError != nil {
...
func (fis *FSIndexSuite) TestGetEvaluationDistributions() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			rules, err := fis.store.ListRules(context.TODO(), storage.NewListRequest(storage.NewResource(tc.namespace, tc.flagKey)))
			require.NoError(t, err)
...
func (fis *FSIndexSuite) TestListAndGetRollouts() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			rollouts, err := fis.store.ListRollouts(context.TODO(), storage.NewListRequest(storage.NewResource(tc.namespace, tc.flagKey),
				storage.ListWithQueryParamOptions[storage.ResourcePredicate](storage.WithPageToken(tc.pageToken))))
			if tc.listError != nil {
				assert.EqualError(t, err, tc.listError.Error())
				return
			}
...
func (fis *FSWithoutIndexSuite) TestListFlags() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			flags, err := fis.store.ListFlags(context.TODO(), storage.NewListRequest(storage.NewNamespace(tc.namespace),
				storage.ListWithQueryParamOptions[storage.NamespacePredicate](storage.WithLimit(5), storage.WithPageToken(tc.pageToken))))
			if tc.listError != nil {
				assert.EqualError(t, err, tc.listError.Error())
				return
...
func (fis *FSWithoutIndexSuite) TestListNamespaces() {
	...
	namespaces, err := fis.store.ListNamespaces(context.TODO(), storage.NewListRequest(storage.ReferencePredicate{},
		storage.ListWithQueryParamOptions[storage.ReferencePredicate](storage.WithLimit(3), storage.WithPageToken("0"))))
	require.NoError(t, err)
...
func (fis *FSWithoutIndexSuite) TestListSegments() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			flags, err := fis.store.ListSegments(context.TODO(), storage.NewListRequest(storage.NewNamespace(tc.namespace),
				storage.ListWithQueryParamOptions[storage.NamespacePredicate](storage.WithLimit(5), storage.WithPageToken(tc.pageToken))))
			if tc.listError != nil {
				assert.EqualError(t, err, tc.listError.Error())
				return
...
func TestFS_Empty_Features_File(t *testing.T) {
	...
	defaultList := storage.NewListRequest(storage.NewNamespace(storage.DefaultNamespace))
	...
	defaultListByFlag := storage.NewListRequest(storage.NewResource(storage.DefaultNamespace, "no-flag"))
...
func TestFS_YAML_Stream(t *testing.T) {
	...
	ns, err := ss.ListNamespaces(context.TODO(), storage.NewListRequest(storage.ReferencePredicate{}))
	require.NoError(t, err)
	...
	var (
		listFootball = storage.NewListRequest(storage.NewNamespace("football"))
		listFruit    = storage.NewListRequest(storage.NewNamespace("fruit"))
	)
