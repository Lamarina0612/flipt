...
func (s *DBTestSuite) TestListSegments() {
	...
	_, err := s.store.ListSegments(context.TODO(), storage.NewListRequest(storage.NewNamespace(storage.DefaultNamespace), storage.ListWithQueryParamOptions[storage.NamespacePredicate](storage.WithPageToken("Hello World"))))
	assert.EqualError(t, err, "pageToken is not valid: \"Hello World\"")

	res, err := s.store.ListSegments(context.TODO(), storage.NewListRequest(storage.NewNamespace(storage.DefaultNamespace)))
	require.NoError(t, err)
	got := res.Results
...
func (s *DBTestSuite) TestListSegmentsNamespace() {
	...
	res, err := s.store.ListSegments(context.TODO(), storage.NewListRequest(storage.NewNamespace(s.namespace)))
	require.NoError(t, err)
	got := res.Results
	assert.NotZero(t, len(got))
...
func (s *DBTestSuite) TestListSegmentsPagination_LimitOffset() {
	...
	// get middle segment
	res, err := s.store.ListSegments(context.TODO(), storage.NewListRequest(storage.NewNamespace(storage.DefaultNamespace), storage.ListWithQueryParamOptions[storage.NamespacePredicate](storage.WithOrder(storage.OrderDesc), storage.WithLimit(1), storage.WithOffset(1))))
	require.NoError(t, err)
	...
	res, err = s.store.ListSegments(context.TODO(), storage.NewListRequest(storage.NewNamespace(storage.DefaultNamespace), storage.ListWithQueryParamOptions[storage.NamespacePredicate](storage.WithOrder(storage.OrderDesc), storage.WithLimit(1))))
	require.NoError(t, err)
	...
	res, err = s.store.ListSegments(context.TODO(), storage.NewListRequest(storage.NewNamespace(storage.DefaultNamespace), storage.ListWithQueryParamOptions[storage.NamespacePredicate](storage.WithOrder(storage.OrderDesc), storage.WithLimit(1), storage.WithOffset(2))))
	require.NoError(t, err)
	...
	// get all segments
	res, err = s.store.ListSegments(context.TODO(), storage.NewListRequest(storage.NewNamespace(storage.DefaultNamespace), storage.ListWithQueryParamOptions[storage.NamespacePredicate](storage.WithOrder(storage.OrderDesc))))
	require.NoError(t, err)
...
func (s *DBTestSuite) TestListSegmentsPagination_LimitWithNextPage() {
	...
	res, err := s.store.ListSegments(context.TODO(), storage.NewListRequest(storage.NewNamespace(storage.DefaultNamespace), storage.ListWithQueryParamOptions[storage.NamespacePredicate](opts...)))
	require.NoError(t, err)
...
func (s *DBTestSuite) TestListSegmentsPagination_FullWalk() {
	...
	req := storage.NewListRequest(
		storage.NewNamespace(namespace),
		storage.ListWithQueryParamOptions[storage.NamespacePredicate](storage.WithLimit(pageSize)),
	)
...
func BenchmarkListSegments(b *testing.B) {
	...
	req := storage.NewListRequest(storage.NewNamespace(storage.DefaultNamespace))
