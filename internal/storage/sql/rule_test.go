...
func (s *DBTestSuite) TestListRules() {
	...
	_, err = s.store.ListRules(context.TODO(),
		storage.NewListRequest(
			storage.NewResource(storage.DefaultNamespace, flag.Key),
			storage.ListWithQueryParamOptions[storage.ResourcePredicate](storage.WithPageToken("Hello World")),
		),
	)
	assert.EqualError(t, err, "pageToken is not valid: \"Hello World\"")

	res, err := s.store.ListRules(context.TODO(), storage.NewListRequest(storage.NewResource(storage.DefaultNamespace, flag.Key)))
	require.NoError(t, err)
...
func (s *DBTestSuite) TestListRulesNamespace() {
	...
	res, err := s.store.ListRules(context.TODO(), storage.NewListRequest(storage.NewResource(s.namespace, flag.Key)))
	require.NoError(t, err)
...
func (s *DBTestSuite) TestListRulesPagination_LimitOffset() {
	...
	res, err := s.store.ListRules(context.TODO(),
		storage.NewListRequest(
			storage.NewResource(storage.DefaultNamespace, flag.Key),
			storage.ListWithQueryParamOptions[storage.ResourcePredicate](storage.WithLimit(1), storage.WithOffset(1)),
		))
	require.NoError(t, err)
...
func (s *DBTestSuite) TestListRulesPagination_LimitWithNextPage() {
	...
	res, err := s.store.ListRules(context.TODO(),
		storage.NewListRequest(
			storage.NewResource(storage.DefaultNamespace, flag.Key),
			storage.ListWithQueryParamOptions[storage.ResourcePredicate](opts...),
		))
	require.NoError(t, err)
...
func (s *DBTestSuite) TestListRulesPagination_FullWalk() {
	...
	resp, err := s.store.ListRules(ctx,
		storage.NewListRequest(
			...
			storage.ListWithQueryParamOptions[storage.ResourcePredicate](
