...
func (s *DBTestSuite) TestListFlags() {
	...
	_, err := s.store.ListFlags(context.TODO(), storage.NewListRequest(
		storage.NewNamespace(storage.DefaultNamespace),
		storage.ListWithQueryParamOptions[storage.NamespacePredicate](storage.WithPageToken("Hello World"))),
	)
	assert.EqualError(t, err, "pageToken is not valid: \"Hello World\"")

	res, err := s.store.ListFlags(context.TODO(), storage.NewListRequest(storage.NewNamespace(storage.DefaultNamespace)))
	require.NoError(t, err)
...
func (s *DBTestSuite) TestListFlagsNamespace() {
	...
	res, err := s.store.ListFlags(context.TODO(), storage.NewListRequest(storage.NewNamespace(s.namespace)))
	require.NoError(t, err)
...
func (s *DBTestSuite) TestListFlagsPagination_LimitOffset() {
	...
	req := storage.NewListRequest(
		storage.NewNamespace(storage.DefaultNamespace),
		storage.ListWithQueryParamOptions[storage.NamespacePredicate](
...
func (s *DBTestSuite) TestListFlagsPagination_LimitWithNextPage() {
	...
	req := storage.NewListRequest(
		storage.NewNamespace(storage.DefaultNamespace),
		storage.ListWithQueryParamOptions[storage.NamespacePredicate](storage.WithOrder(storage.OrderDesc), storage.WithLimit(1)),
	)
	res, err := s.store.ListFlags(context.TODO(), req)
	require.NoError(t, err)
