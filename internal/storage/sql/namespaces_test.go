...
func (s *DBTestSuite) TestListNamespaces() {
	...
	_, err := s.store.ListNamespaces(context.TODO(), storage.NewListRequest(
		storage.ReferencePredicate{},
		storage.ListWithQueryParamOptions[storage.ReferencePredicate](
			storage.WithPageToken("Hello World"),
		),
	))
	assert.EqualError(t, err, "pageToken is not valid: \"Hello World\"")

	res, err := s.store.ListNamespaces(context.TODO(), storage.NewListRequest(
		storage.ReferencePredicate{},
	))
	require.NoError(t, err)
...
func (s *DBTestSuite) TestListNamespacesPagination_LimitWithNextPage() {
	...
	res, err := s.store.ListNamespaces(context.TODO(), storage.NewListRequest(
		storage.ReferencePredicate{},
		storage.ListWithQueryParamOptions[storage.ReferencePredicate](opts...),
	))
	require.NoError(t, err)
